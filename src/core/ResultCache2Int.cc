...
#pragma region INTERNALS
    ...
    ResultCache2::ResultCache2(result_cache_parameter_t& parameter_info) noexcept
        ...
        ddf_write_log(
            new DdfWriteLog(parameter_info.vector_intopk, 16, parameter_info.risk_threshold)
        ),
        cache_lock(false)
    {
        ...
        try_read_cnt = 0;
        global_similarity_threshold = 0.0f;

        inst_distance_function = nullptr;
    ...
    ResultCache2::_evictVecs2(size_t to_evicts, std::vector<vector_id_t>& evicted_list) noexcept
    {
        ...
        while (eviction_size < to_evicts)
        {
            ...
            // Unlike the TopKache, it could be the one erased from the dropout.
            // In this case, we move to the next candidate.
            if (evict_candidate_entry == nullptr)
            {
                // Check if the eviction strategy has more candidates
                if (eviction_strategy->getCurrSize() == 0)
                    break;

                // If the eviction strategy has more candidates, we continue to the next candidate.
                continue;
            }
            ...
            if (evict_candidate_entry->checkpoint != nullptr)
            {
                ...
                unseen = ddf_write_log->getDistance(evict_candidate_entry->checkpoint);
            ...
            // ddf_write_log->removeFromRr(reinterpret_cast<rev_centry_t*>(evict_candidate_entry));

            lookup_table->map.erase(evict_candidate_id);
            repr_entry_cnt = eviction_strategy->getCurrSize();

            // ddf_write_log->decreaseCEntryDdf(
        ...
        // ddf_write_log->safeTrims(ddf_write_log->getHead());
    }
    ...
    ) noexcept
    {
        ...
        saved_entry = reinterpret_cast<result_cache_entry_t*>(ddf_write_log->getNextRr());
        ...
        write_log_entry_t* new_ckpt = ddf_write_log->sweepLEntryBatch(
        ...
        // Sort the found entries by the distance, in descending order
        std::sort(
            found_entries.begin(), found_entries.end(),
            [](const close_candidates_t& a, const close_candidates_t& b) { return a.distance > b.distance; }
        );
        ...
        if (found_entries.size() > 0)
        {
            ...
            float max_distance = distance_function(
                saved_entry->query_vector->getVecData(), 
                found_entries.back().entry->vector.getVecData(), 
                vector_pool->getVecDataSize()
            );

            saved_entry->max_distance = max_distance;

            size_t vector_data_size = vector_pool->getVecDataSize();
            for (int k = saved_entry->vector_list_size - 1; k >= 0; k--)
            {   
                // The vector may be the deleted ones.
                if (!(saved_entry->vector_slot_ref_list[k]->isValid()))
                    continue;

                close_candidates_t found_entry = found_entries.back();

                vector_id_t delete_vector_id = saved_entry->vector_slot_ref_list[k]->getVecId();

                vector_id_t found_vector_id = found_entry.entry->vector.getVecId();
                vector_data_t* found_vector_data = found_entry.entry->vector.getVecData();

                std::uint64_t aux_data_1 = found_entry.entry->vector.getAuxData1();
                std::uint64_t aux_data_2 = found_entry.entry->vector.getAuxData2();
                ...
                // First argument: raw vector data,
                // Second argument: the size of the vector data
                // Convert as you wish.

                saved_entry->vector_slot_ref_list[k] = sub_vec;
        ...
        // std::cout << write_log->toString() << std::endl;
        ...
        ddf_write_log->decreaseCEntryDdf(0, distance, repr_entry_cnt);
    }
#pragma endregion
}