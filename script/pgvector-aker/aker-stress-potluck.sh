...
write_topkache_config() {
    ...
    # risk_threshold : Potliuck 에서는 dropout 확률 e.g. 25 --> 25% dropout
    echo "{
    ...
    for repeat_index in {1..1}; do
        ...
        for vector_pool_size_value in "${vector_pool_size[@]}"; do
            for invalidate_value in "${invalidate_percentage[@]}"; do
                ...
                result_file_dir="$working_path/4-results/potluck/${workload_name}/${vector_pool_size_value}-${invalidate_value}-${repeat_index}"
                mkdir -p "$result_file_dir"

                mv report.csv "$result_file_dir/report.csv"
                mv search-results.pkl "$result_file_dir/search-results-${invalidate_value}.pkl"
