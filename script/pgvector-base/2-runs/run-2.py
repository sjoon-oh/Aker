...
if __name__ == "__main__":
    ...
    for i, request in enumerate(trace):
        ...
        if percentage >= show_progress_percentage:
            logging.info(f"Progress: {percentage:.2f}% comspleted.")
            show_progress_percentage += 1

            elapsed = request_end - workload_start
            estimated_remaining = (elapsed / percentage) * (100 - percentage) if percentage > 0 else 0
            logging.info(f"Estimated remaining time: {estimated_remaining:.2f} seconds.")
            logging.info(f"Current QPS: {(i + 1) / (request_end - workload_start)}")
